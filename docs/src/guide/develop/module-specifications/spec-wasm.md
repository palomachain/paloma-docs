...
Smart contracts are autonomous agents that can interact with other entities on the Paloma blockchain, such as human-owned accounts, validators, and other smart contracts. Each smart contract has:
...
In addition to the regular gas fees incurred from creating the transaction, Paloma also calculates a separate gas when executing smart contract code. This is tracked by the **gas meter**, which is during the execution of every opcode and gets translated back to native Paloma gas via a constant multiplier (currently set to 100).

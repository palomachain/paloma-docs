...
This document explains how to perform tasks related to working with smart contracts with Paloma.js.
...
```ts
import { LCDClient, MsgStoreCode, MnemonicKey, isTxError } from '@Paloma-money/Paloma.js';
...
For Paloma smart contracts, there is a distinction between uploading contract code and creating a contract. This allows multiple contracts to share the same code if there are only minor variations in their logic which can be configured at contract creation. This configuration is passed in an **InitMsg**, and provides the initial state for the contract.
...
```ts
import { MsgInstantiateContract } from '@Paloma-money/Paloma.js';
